/**
 * Client-side circuit breaker for Gemini /api/gemini/health
 */
...
export function isGeminiAvailableSync(): boolean {
  if (typeof window === 'undefined') return !!process.env.GEMINI_API_KEY?.trim();
  if (isCacheValid()) return cached!.available;
  return false;
}

export function getGeminiCircuitReason(): AIHealthReason | undefined {

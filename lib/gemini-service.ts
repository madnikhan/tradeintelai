/**
 * Gemini AI Service
 * Handles Gemini-powered analysis explanations and insights
 */

import { MarketAnalysis } from './ai-trading-engine';
import { getAuth } from 'firebase/auth';
import { getApp } from './firebase/config';
import { formatGeminiProxyErrorForUser } from './gemini-api-errors';
import {
...
/**
 * Check if Gemini is configured and available (client uses circuit breaker from /api/gemini/health).
 */
export function isGeminiConfigured(): boolean {
  if (typeof window === 'undefined') {
    return !!process.env.GEMINI_API_KEY?.trim();
  }
  return isGeminiAvailableSync();
}

async function assertGeminiAvailable(): Promise<boolean> {
  if (typeof window === 'undefined') {
    return !!process.env.GEMINI_API_KEY?.trim();
  }
...
function handleGeminiProxyFailure(status: number, errorData: Record<string, unknown>): never {
  throw new Error(formatGeminiProxyErrorForUser(status, errorData));
}
...
async function callGeminiApi(
  ...
  if (!response.ok) {
    ...
    handleGeminiProxyFailure(response.status, errorData as Record<string, unknown>);
  }
...
async function getAuthToken(): Promise<string | null> {
  ...
  try {
    ...
    const app = getApp();
    if (!app) {
      console.warn('⚠️ Firebase not initialized - Gemini API calls will fail');
      return null;
    }
...
export async function generateAnalysisExplanation(
  ...
  if (!(await assertGeminiAvailable())) {
    console.warn('⚠️ Gemini unavailable — skipping AI explanation');
    return null;
  }
  ...
  try {
    ...
    const gptText = await callGeminiApi(
      {
        mode: 'text',
        system:
...
export async function enhanceSentimentAnalysis(newsArticles: string[]): Promise<number | null> {
  if (newsArticles.length === 0 || !(await assertGeminiAvailable())) {
    return null;
  }
...
Format: "Score: X\nExplanation: Y"`;
    ...
    console.error('Gemini sentiment enhancement failed:', error);
    return null;
  }
}
...
export async function analyzeChartImage(
  ...
  if (!(await assertGeminiAvailable())) {
    console.warn('⚠️ Gemini unavailable — skipping chart vision analysis');
    return null;
  }
  ...
  try {
    ...
    try {
      ...
      if (isDevOrTest) {
        console.warn('⚠️ Firebase auth failed in dev/test - proceeding without auth token for Gemini vision analysis');
...
export { convertToStructureAnalysis as convertToGeminiStructureAnalysis } from './ai-structure-analysis';
/** @deprecated Use convertToStructureAnalysis from ai-service */
export { convertToStructureAnalysis as convertToGPTStructureAnalysis } from './ai-structure-analysis';
